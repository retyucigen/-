...
void ChatClient::receiveMessages() 
{
    ...
    while (isConnected) 
    {
        ...
        if (receivedMsg.find(username + ": ") == 0) 
        {
            std::cout << COLOR_GREEN << receivedMsg << COLOR_RESET << std::endl;
        } 
        else if (receivedMsg.find("系统: ") == 0) 
        {
            // 系统消息 - 青色
            std::cout << COLOR_CYAN << receivedMsg << COLOR_RESET << std::endl;
        } 
        else 
        {
            // 其他人发送的消息 - 黄色
            std::cout << COLOR_YELLOW << receivedMsg << COLOR_RESET << std::endl;
        }
    }
}
